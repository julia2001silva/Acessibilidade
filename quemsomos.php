<!DOCTYPE html>
<html lang="pt-BR">

<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Encode+Sans:wght@300;400;700&family=M+PLUS+1+Code:wght@300;400;700&family=Montserrat:wght@200;400;500;700&family=Poppins:wght@300;400;500;600;900&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="css/style.css">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"
    integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">


  <title>Quem Somos</title>
</head>

<body>
  <script type="text/javascript" src="bootstrap.min.js"></script>

  <div class="container-fluid">
    <nav>
      <div class="row">
        <?php include('includes/nav.php'); ?>
      </div>
    </nav>
  </div>
  <br>

  <header id="content" class="container">
    <h1>Quem somos</h1>
    <p>Somos alunos da Fatec São Roque e esse portal tem como principal objetivo incentivar outros programadores a
      deixarem seus sites acessiveis. Através de dicas que podem ser encontradas na página acessibilidade, é explicado
      diversos fatores que qualquer site deve ter para ser acessivel. </p>
  </header>
  

  <main>
    <article>
      <div class="container">

        <div class="card mb-3">
          <div class="row g-0">
            <div class="col-xl-10 col-md-8 col-sm-12">
              <div class="card-body">
                <h5 class="card-title">Julia Alves da Silva (RA: 2650832023015)</h5>
                <p class="card-text"> Responsavel pelo desenvolvimento das páginas(<a href="index.php">Home</a>, <a href="quemsomos.php">Sobre</a>). É estudante, tem 20 anos, mora em São Roque.
                  Começou estudar programação no ano de 2019. Gosta de ler e colecionar livros.</p>
              </div>
            </div>
            <div class="col-xl-2 col-md-4 col-sm-12">
              <img src="image/julia.jpg" class="img-fluid" alt="Foto de Julia umas das desenvolvedoras do site">
            </div>
            <div class="card-footer">
              <a class="text-decoration-none" href="https://www.facebook.com/profile.php?id=100055771643749" title="Ir para o perfil do facebook de Julia">
                <svg 
                  alt="perfil do facebook de Julia"
                  xmlns="http://www.w3.org/2000/svg" 
                  width="16" 
                  height="16"
                  fill="currentColor" 
                  class="bi bi-facebook text-muted"
                  viewBox="0 0 16 16">
                  <path
                      d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z" />
                </svg> 
              </a>
                
              <a class="text-decoration-none" href="https://www.linkedin.com/in/julia-alves-da-silva-3865841b4/" title="Ir para o perfil do linkedin de Julia">
                <svg 
                  alt="perfil do linkedin de Julia"
                  xmlns="http://www.w3.org/2000/svg" 
                  width="16" 
                  height="16" 
                  fill="currentColor"
                  class="bi bi-linkedin text-muted" 
                  viewBox="0 0 16 16">
                  <path
                      d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z" />
                </svg>
              </a>

              <a class="text-decoration-none" href="https://github.com/julia2001silva" title="Ir para o perfil do github de Julia">
                <svg 
                alt="perfil do git de Julia"
                  xmlns="http://www.w3.org/2000/svg" 
                  width="16" 
                  height="16" 
                  fill="currentColor" 
                  class="bi bi-github text-muted" 
                  viewBox="0 0 16 16">
                  <path
                      d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                </svg>
              </a>
              
            </div>
          </div>
        </div>


        <div class="card mb-3">
          <div class="row g-0">
            <div class=" col-xl-2 col-md-4 col-sm-12">
              <img src="./image/lucas.jpeg" class="img-fluid" alt="Foto de Lucas um dos desenvolvedores do site"></div>
            <div class="col-xl-10 col-md-8 col-sm-12">
              <div class="card-body">
                <h5 class="card-title">Lucas Franklin Targa (RA:2650832023022)</h5>
                <p class="card-text">Entusiasta de programação e graduando em Sistemas para Internet na FATEC-SR.
                    Possuo conhecimento em HTML5, CSS3, JavaScript e o básico de PHP. Responsavel pelo desenvolvimento da página( <a href="acessibilidade.php">Acessibilidade</a>).</p>
              </div>
            </div>

            <div class="card-footer">
            <a class="text-decoration-none" href="https://www.facebook.com/lucas.targa.520/" title="Ir para o perfil do facebook de Lucas">
                <svg 
                  alt="perfil do facebook de Lucas"
                  xmlns="http://www.w3.org/2000/svg" 
                  width="16" 
                  height="16"
                  fill="currentColor" 
                  class="bi bi-facebook text-muted"
                  viewBox="0 0 16 16">
                  <path
                      d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z" />
                </svg> 
              </a>
                
              <a class="text-decoration-none"  href="https://www.linkedin.com/in/lucas-targa/" title="Ir para o perfil do linkedin de Lucas">
                <svg 
                  alt="perfil do linkedin de Lucas"
                  xmlns="http://www.w3.org/2000/svg" 
                  width="16" 
                  height="16" 
                  fill="currentColor"
                  class="bi bi-linkedin text-muted" 
                  viewBox="0 0 16 16">
                  <path
                      d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z" />
                </svg>
              </a>

              <a class="text-decoration-none" href="https://github.com/Lucastarga" title="Ir para o perfil do github de Lucas">
                <svg 
                alt="perfil do git de Lucas"
                  xmlns="http://www.w3.org/2000/svg" 
                  width="16" 
                  height="16" 
                  fill="currentColor" 
                  class="bi bi-github text-muted" 
                  viewBox="0 0 16 16">
                  <path
                      d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                </svg>
              </a>
              
            </div>
          </div>
        </div>

        <div class="card mb-3">
          <div class="row g-0">
            <div class="col-xl-10 col-md-8 col-sm-12">
              <div class="card-body">
                <h5 class="card-title">Wesley Honorio Gomes (RA: 2650832023017)</h5>
                <p class="card-text">Residente em Mairinque, com 23 anos, estudante do curso de Sistemas para Internet, na FATEC São Roque, onde expressa interesses pelas áreas de Banco de dados e Infraestrutura. Responsavel pelo desenvolvimento das páginas( <a href="contato.php">Contato</a>, <a href="normas.php">Normas de Uso</a>).</p>
              </div>
            </div>
            <div class="col-xl-2 col-md-4 col-sm-12">
              <img src="./image/wesley.jpeg" class="img-fluid" alt="Foto de Wesley um dos desenvolvedores do site">
            </div>
            <div class="card-footer">
            <a class="text-decoration-none" href="https://www.facebook.com/wesley.honorio.1238" title="Ir para o perfil do facebook de Wesley">
                <svg 
                  alt="perfil do facebook de Wesley"
                  xmlns="http://www.w3.org/2000/svg" 
                  width="16" 
                  height="16"
                  fill="currentColor" 
                  class="bi bi-facebook text-muted"
                  viewBox="0 0 16 16">
                  <path
                      d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z" />
                </svg> 
              </a>
                
              <a class="text-decoration-none" href="https://www.linkedin.com/in/wesley-hon%C3%B3rio-gomes-87b76a142/" title="Ir para o perfil do linkedin de Wesley">
                <svg 
                  alt="perfil do linkedin de Wesley"
                  xmlns="http://www.w3.org/2000/svg" 
                  width="16" 
                  height="16" 
                  fill="currentColor"
                  class="bi bi-linkedin text-muted" 
                  viewBox="0 0 16 16">
                  <path
                      d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z" />
                </svg>
              </a>

              <a class="text-decoration-none"  href="https://github.com/WesleyHonorio" title="Ir para o perfil do github de Wesley">
                <svg 
                alt="perfil do git de Wesley"
                  xmlns="http://www.w3.org/2000/svg" 
                  width="16" 
                  height="16" 
                  fill="currentColor" 
                  class="bi bi-github text-muted" 
                  viewBox="0 0 16 16">
                  <path
                      d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                </svg>
              </a>
              
            </div>
          </div>
        </div>
      </div>
    </article>
  </main>

  <footer class="col-12">
    <?php include('includes/footer.php'); ?>
  </footer>

  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"
    integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous">
  </script>
</body>

</html>